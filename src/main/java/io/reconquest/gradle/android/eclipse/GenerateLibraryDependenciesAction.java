...
public class GenerateLibraryDependenciesAction implements Action<Classpath> {
  ...
  private Stream<ClasspathEntry> explodeAarJarFiles(Library aarLibrary) {
    ...
    File targetFolder =
        new File(new File(new File(project.getProjectDir(), "build"), "exploded-aars"), jarId);
    if (!targetFolder.exists()) {
      if (!targetFolder.mkdirs()) {
        throw new RuntimeException(
            format("Cannot create folder: {0}", targetFolder.getAbsolutePath()));
      }
  ...
  private void ensureParentFolderExists(File targetFile) {
    ...
    if (!parentFolder.exists()) {
      if (!parentFolder.mkdirs()) {
        throw new RuntimeException(
            format("Cannot create folder: {0}", parentFolder.getAbsolutePath()));
      }
    }
  }
  ...
  private void copy(ZipFile zipFile, File targetFile, ZipEntry entry) {
    try (InputStream inputStream = zipFile.getInputStream(entry)) {
      ...
      throw new RuntimeException(
          format(
              "Cannot write entry to file: {0}: {1}", e.getMessage(), targetFile.getAbsolutePath()),
          e);
    }
  }
}
